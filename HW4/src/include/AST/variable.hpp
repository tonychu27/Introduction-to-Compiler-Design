...
class VariableNode final : public AstNode {
  private:
    ...
    shared_ptr<ConstantValueNode> m_constant_value_node_ptr;

  public:
    ~VariableNode() = default;
    VariableNode(const uint32_t line, const uint32_t col,
                 const string &p_name, const PTypeSharedPtr &p_type,
                 const shared_ptr<ConstantValueNode> &p_constant_value_node)
    ...
    int checkInvalidDimensions() const{ return m_type->checkInvalidDimensions(); }
