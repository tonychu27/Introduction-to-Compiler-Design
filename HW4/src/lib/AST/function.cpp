...
getParametersTypeString(const FunctionNode::DeclNodes &p_parameters) {
    ...
    if (!p_parameters.empty()) { type_string.erase(type_string.end() - 2, type_string.end()); }
...
const char *FunctionNode::getPTypeCString() const { return m_ret_type->getPTypeCString(); }

const char *FunctionNode::getArgumentCString() const{
    if(m_argument_string==""){
        getPrototypeCString();
        size_t start = m_prototype_string.find('(')+1;
        size_t end = m_prototype_string.find(')')-1;
...
void FunctionNode::visitChildNodes(AstNodeVisitor &p_visitor) {
    ...
    if (m_body) { visit_ast_node(m_body); }
}
